...
inline bool isUpper(std::string const &s) {
	return std::all_of(s.begin(), s.end(), static_cast<bool (*)(char const &)>(&isUpper));
}

inline bool isLower(std::string const &s) {
	return std::all_of(s.begin(), s.end(), static_cast<bool (*)(char const &)>(&isLower));
}
...
inline std::string path_extension(const std::string &name) {
	auto e = utils::path(name).extension();
    if(e[0] == '.')
        return e.substr(1);
    return e;
}
...
static inline const uint32_t crctab[] = {
    0x0,        0x04c11db7, 0x09823b6e, 0x0d4326d9, 0x130476dc, 0x17c56b6b, 0x1a864db2,
